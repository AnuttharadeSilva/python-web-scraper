...
csv_file = open('ovp_1min_mp4.csv', 'w')
csv_writer = csv.writer(csv_file)
csv_writer.writerow(['title', 'year', 'description','genre','keywords','duration','popularity','video_url','image_path' ])

with open('ovp_1min_mp4.html') as html_file:
...
for result in results.find_all('tr'):
    ...
    if td != None and td_img!= None:
        ...
        parts = re.split(r'(\(\d{4}\))|(?:Genre:\s)|(?:Keywords:\s)|(?:Duration:\s)|(?:Popularity\s\(downloads\):\s)', td_text)
        ...
        csv_writer.writerow([title, year, description, genre, keywords, duration, popularity, video_url, img_file])
